...
export default class RoleService implements IRoleService {
  ...
  public async getAllRoles(): Promise<Result<IRoleDTO[]>> {
    try {
      const truckList = await this.roleRepo.findAll(); 

      if (truckList === null) {
        return Result.fail<IRoleDTO[]>("Trucks not found");
      }

      const result = truckList.map((truckList) => RoleMap.toDTO(truckList) as IRoleDTO);
      return Result.ok<IRoleDTO[]>(result);
