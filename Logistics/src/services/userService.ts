...
export default class UserService implements IUserService{
  ...
  public async SignIn(email: string, password: string): Promise<Result<{ userDTO: IUserDTO, token: string }>> {
    ...
    if (validPassword) {
      ...
      return Result.fail<{userDTO: IUserDTO, token: string}>({ 'www': String});
    }
  }
  ...
  public async getUser( userDTO: IUserDTO): Promise<Result<IUserDTO>> {
    try {
  ...
  public async getAllUsers(): Promise<Result<IUserDTO[]>> {
    try {
      ...
      const result = users.map((users) => UserMap.toDTO(users) as IUserDTO);
  ...
  public async anonymizeUser(userDTO: IUserDTO): Promise<Result<IUserDTO>> {
    try {
      const user = await this.userRepo.findByEmail(userDTO.email);
      if (user === null) {
        ...
        const emailRandom = randomString + '@' + randomString2 + '.' + randomString3;
        const emailUser = new UserEmail({value: emailRandom});
