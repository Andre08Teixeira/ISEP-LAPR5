...
entrega(1, 20221205, 200, 1, 8, 10).
entrega(2, 20221205, 150, 9, 7, 9).
entrega(3, 20221205, 100, 3, 5, 7).
entrega(4, 20221205, 120, 8, 6, 8).
entrega(5, 20221205, 300, 11, 15, 20).
entrega(6, 20221205, 300, 4, 15, 20).
entrega(7, 20221205, 300, 4, 15, 20).

entrega(8, 20221205, 300, 4, 15, 20).

entrega(9, 20221205, 300, 4, 15, 20).

entrega(10, 20221205, 300, 4, 15, 20).
...
%Minorante
menorTempoParaID([R],R):-!.
menorTempoParaID([H|T],R):-menorTempoParaID(T,R1),((H<R1, R is H);R is R1),!.

% X � o id, Z o resultado ou seja minorante de 1 entrega, R a lista dos
% tempos para esse Id, Y � o menor tempo para esse ID, U � a massa da
% entrega, K o tempo de retirada,
%
%
todosIdsDasEntregas(X):-findall(D, entrega(_,_,_,D,_,_), X).

contas(U,Y,R):- R is (Y*((7500+(870-U))/11800)).

%viagensQvaoparaOId
viagensParaId(X,Z):-(findall(D,dadosCam_t_e_ta(_,_,X,D,_,_),R)), menorTempoParaID(R,Y),entrega(_,_,U,X,_,K), contas(U,Y,P), Z is P+K.
...
%auxiliar de verificar maior numa lista
maiorTempoPercursoParaIdMaj([R],R):-!.
maiorTempoPercursoParaIdMaj([H|T],R):-maiorTempoPercursoParaIdMaj(T,R1),((H>R1, R is H);R is R1),!.


listaTemposDepoisContas(_,[],[]):-!.
listaTemposDepoisContas(U,[H|Cauda],[H1|Cauda1]):-
contas(U,H,P),
H1 is P,
listaTemposDepoisContas(U,Cauda,Cauda1).


somatorio([],0).
somatorio([H|T],R):-somatorio(T,R1), R is R1+H.

%Obter maior tempo da parcela trajeto
viagensParaIdMajorante(X,Z):-(findall(D,dadosCam_t_e_ta(_,_,X,D,_,_),R)),
findall(Tempo,entrega(_,_,_,_,_,Tempo),TemposEntrega),
...
listaTemposAdicionais(X,L):-findall(D,dadosCam_t_e_ta(_,_,X,_,_,D), L).
%Obter maior tempo da parcela tempo adicional
tempoAdicional(X,TempoAd):-listaTemposAdicionais(X,List), maiorTempoPercursoParaIdMaj(List,TempoAd).
...
contasTempo(MedioCarregar, R):-Subtracao is (MedioCarregar-64), R is ((Subtracao*60)/48).

listaAtestar(X,L):-findall(D,dadosCam_t_e_ta(_,_,X,_,D,_), L).
%Obter maior tempo da parcela tempo adicional
...
majorante(M):-todosIdsDasEntregas(X),recursaoMaj(X,MedioTrajeto),recursaoTempoCarregar(X,MedioCarregar), contasTempo(MedioCarregar,R),  M is MedioTrajeto + R.
%Majorante
